"""Source-backed CCF contract for controlled system maintenance."""

from security_lakehouse.safeguards import load_safeguards, safeguards_by_requirement


def test_controlled_maintenance_uses_the_nist_800_171_crosswalk() -> None:
    safeguards = {entry["safeguard_id"]: entry for entry in load_safeguards()["safeguards"]}
    safeguard = safeguards["SG-MAINTENANCE-001"]

    assert safeguard["title"] == "Controlled system maintenance"
    assert {
    ...
    assert safeguard["mapping_source"] == {
        ...
        "locator": "Appendix D, Table D-7, requirement 3.7.1 maps to MA-2",
    }

    mappings = safeguards_by_requirement()
    assert mappings["CMMC-3.7.1"] == ["SG-MAINTENANCE-001"]
    assert mappings["FEDRAMP-MA-2"] == ["SG-MAINTENANCE-001"]
