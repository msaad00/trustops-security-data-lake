"""Source-backed CCF contract for user and management function separation."""
...
def test_function_separation_uses_the_nist_800_171_crosswalk() -> None:
    safeguards = {entry["safeguard_id"]: entry for entry in load_safeguards()["safeguards"]}
    safeguard = safeguards["SG-FUNCTIONSEPARATION-001"]

    assert safeguard["title"] == "User and management function separation"
    assert {
    ...
    assert safeguard["mapping_source"] == {
        ...
        "locator": "Appendix D, Table D-13, requirement 3.13.3 maps to SC-2",
    }

    mappings = safeguards_by_requirement()
    for control_id in ("CMMC-3.13.3", "FEDRAMP-SC-2"):
        assert mappings[control_id] == ["SG-FUNCTIONSEPARATION-001"]


def test_function_separation_closes_two_gaps_without_overstating_attestation() -> None:
    payload = load_safeguards()
    payload["safeguards"] = [
        entry for entry in payload["safeguards"] if entry["safeguard_id"] != "SG-FUNCTIONSEPARATION-001"
    ]
