...
| Open the workbench | `security-lakehouse serve --lake build/lakehouse --server --allow-insecure-no-auth --port 8787` | server-mode console and API                                    | open `http://127.0.0.1:8787/console/dashboard/` |
...
| Workflow canvas                | Typed workflow graph model with trigger, condition, assignment, snapshot, webhook, Slack, Jira, and trust-share actions; UI canvas and API endpoints persist and run workflow versions.                                                                             | `src/security_lakehouse/workflows.py`, `app/web/src/app/automation/page.tsx`, `tests/test_workflows.py`                                           |
...
| Enterprise connector ingestion    | Direct production ingestion for every source system in the catalog.                                          | GitHub, AWS, and Okta runners are executable with fixtures or scoped credentials. Other catalog sources remain planned or existing-lake-read contracts.                                                      |
| Full framework coverage           | Exhaustive official-control evaluation for every listed framework.                                           | The repo ships 34 seeded source-linked controls with reviewed mappings across 8 frameworks. Neutral framework labels show source-linked scope and readiness-gate status; they do not indicate certification. |
